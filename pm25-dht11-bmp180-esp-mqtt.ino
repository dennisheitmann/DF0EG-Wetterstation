...
float temp_dht = (-127.0);
float humi_dht = (-127.0);
...
void sendAPRS() {
  ...
  if ((WiFi.status() == WL_CONNECTED)) {
    ...
    client.printf("%02d", (int)humi_dht);
    client.print("b");
...
void publishSensors() {
  ...
  char humi_dht_str[8];
  dtostrf(humi_dht, 6, 1, humi_dht_str);
  if (! client.publish(mqtt_topic_2, humi_dht_str)) {
  ...
  if (! client.publish(mqtt_topic_3, temp_bmp_str)) {
  ...
  if (! client.publish(mqtt_topic_4, baro_bmp_str)) {
...
void loop(void) {
  ...
  if (duration > (sampletime * 1000000))
  {
    ...
    do {
      ...
      readSensors();
      i++;
    } while ((temp_bmp == -127 || baro_bmp == -127 || humi_dht == -127 || humi_dht > 100) && (i < 5));
